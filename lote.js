...
if(!ACTOS.length) console.log('  (no hay ningún programa cargado)');
  ...
  let salen=0, mal=0, sinQ=0, conActos=0, pruebas=0, nocturnoNinos=0, deTodosNinos=0;
  dias.forEach(f=>{
    [...new Set(ACTOS.filter(a=>a.f===f).map(a=>a.m))].forEach(muni=>{
      [true,false].forEach(ninos=>{
        ...
        a.lista.forEach(x=>{
          ...
          /* `todos` —fuegos, pirotecnia, romería— vale para los dos públicos:
             lo ve el pueblo entero. Es el tercer valor que CLAUDE.md dejaba
             apuntado como «una línea en actosDelDia() y otra aquí». */
          const deTodos=!!ficha&&ficha.q==='todos';
          if(!deTodos && ninos!==esDeNinos) mal++;
          if(deTodos&&ninos) deTodosNinos++;
          if(!ficha||!ficha.q) sinQ++;
          /* con niños, nada de madrugada ni de después de las once — salvo lo
             de todo el pueblo, que a las once son justo los fuegos */
          if(ninos&&ficha&&!deTodos&&ficha.h&&(ficha.h>='23:00'||ficha.h<'07:00')) nocturnoNinos++;
        });
      });
    });
  });
  console.log('  actos cargados            : '+ACTOS.length+
    ...
    ACTOS.filter(a=>a.q==='todos').length+' de todo el pueblo, '+
  ...
  console.log('  OFRECIDO A QUIEN NO TOCA  : '+mal+
    '   (de niños a una pareja, o no marcado de niños a una familia; los «todos» valen para ambos)');
  console.log('  DE TODO EL PUEBLO, a una familia: '+deTodosNinos+
    '   (los fuegos y las romerías, que antes no veía)');
